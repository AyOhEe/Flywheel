	...
	public ItemDisplayVisual(VisualizationContext ctx, Display.ItemDisplay entity, float partialTick) {
		...
		if (itemRenderState == null) {
			...
			currentStack = itemRenderState.itemStack()
					.copy();
			instance = ctx.instancerProvider()
					.instancer(InstanceTypes.TRANSFORMED, ItemModels.get(level, currentStack, itemRenderState.itemTransform(), ItemModels.glowColour(entity)))
					.createInstance();
		}
	...
	public void beginFrame(Context ctx) {
		...
		if (!ItemStack.matches(itemStack, currentStack)) {
			...
			visualizationContext.instancerProvider()
					.instancer(InstanceTypes.TRANSFORMED, ItemModels.get(level, currentStack, object.itemTransform(), ItemModels.glowColour(entity)))
